  ...
  Widget _gestureHandler({required Widget child}) {
    ...
    return Listener(
      onPointerDown: (PointerDownEvent e) {
        ...
        // stop it right here
        if (result.path
            .any((entry) => entry.target.runtimeType == _ForceDraggableWidgetRenderBox)) {
        ...
        // stop it right here
            ...
            .any((entry) => entry.target.runtimeType == _IgnoreDraggableWidgetWidgetRenderBox)) {
...
class PanelScrollPhysics extends ScrollPhysics {
  ...
  double applyPhysicsToUserOffset(ScrollMetrics position, double offset) {
    if (controller?._nowTargetForceDraggable == true) return 0.0;
    return super.applyPhysicsToUserOffset(position, offset);
  }

  @override
  Simulation? createBallisticSimulation(ScrollMetrics position, double velocity) {
    if (controller?._nowTargetForceDraggable == true)
